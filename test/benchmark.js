...
function withJampary(buffer, target, i, j) {
  ...
  let cx = J.add(J.sub(tx, tdx), J.div(J.mul(tdx, [2 * i]), [buffer.width]));
  let cy = J.sub(J.add(ty, tdy), J.div(J.mul(tdy, [2 * j]), [buffer.height]));
  while (iteration++ < maxIteration && J.add(xx, yy)[0] < 4) {
...
function drawSplitTest(calc1, calc2, target) {
  ...
  ctx.fillStyle = '#FFF';
  ctx.fillText(calc1.name.slice(4), canvas.width - 56, 20);
...
window.onload = function() {
  ...
  maxIteration = 1000;
  drawSplitTest(withJampary, withNumber, target);
