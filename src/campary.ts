...
// Algorithm 5 from [1]
// need one array optimization
function mul(X: Array<float>, Y: Array<float>): Array<float> {
  let n = X.length;
  let R = new Array<float>(n);
  let E = new Array<float>(n);
  ...
  for(let j = 1; j < n; j++) {
    ...
    let S = vecSum(vecMix(P, 0, j + 1, E, 0, j*j));
    R[j] = S[0];
    E = S.slice(1, j*j + j + 1);
    E.slice(0, (j + 1) * (j + 1)) = vecMix(E, 0, j*j + j, E2, 0, j);
  }
  for(let i = 1; i < n; i++) R[n] += X[i] * Y[n - i];
  for(let i = 1; i < n * n; i++) R[n] += E[i];
  return R.renormalize(R, k);
}
